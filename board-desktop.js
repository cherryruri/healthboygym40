(() => {
 ...
 shell.before(stories); stories.querySelectorAll('a.mall-story').forEach(a=>{a.href='board.html?view=list'});
 ...
 const footer=document.createElement('footer');footer.className='desktop-community-bottom mall-footer';footer.innerHTML='<strong>HEALTHBOYGYM <small>수내점</small></strong><span>건강한 일상이 시작되는 곳</span><nav aria-label="하단 안내"><a href="index.html#facility">시설 안내</a><a href="index.html#hours">운영시간</a><a href="index.html#location">오시는 길</a></nav>';/* No duplicate footer shortcuts. */
 const sync=()=>{document.body.classList.toggle('desktop-board',media.matches);document.body.classList.toggle('cinema-mobile',!media.matches);if(media.matches){shell.removeAttribute('aria-hidden');document.body.classList.remove('board-header-over-hero');}};sync();media.addEventListener('change',sync);
 ...
 const extra=document.createElement('section');extra.className='cinema-extras';extra.innerHTML='<div class="cinema-event-heading"><h2>이벤트 · 센터 소식</h2><a href="board.html?board=free&category=news&view=list">더보기 ↗</a></div><div class="cinema-events" aria-label="센터 소식 슬라이드"></div><nav class="cinema-event-shortcuts" aria-label="이벤트 바로가기"><a href="board.html?board=free&category=news&view=list">센터 이벤트 ↗</a><a href="board.html?board=free&category=challenge&view=list">바디챌린지 ↗</a><a href="'+booking+'" target="_blank" rel="noopener noreferrer">상담 예약 ↗</a></nav><div class="cinema-notice-heading"><h2>칭찬게시판</h2><a href="board.html?board=free&category=praise&view=list">전체 보기 ↗</a></div><div id="cinemaNotices"></div>';
 /* No duplicate lower event sections. */
 const eventBox=extra.querySelector('.cinema-events');
 const syncMobileTabs=()=>mobileTabs.querySelectorAll('a').forEach((a,i)=>{if(i===Number(document.body.classList.contains('board-list-view')))a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});syncMobileTabs();window.addEventListener('board-view-change',syncMobileTabs);
 const eventItems=[['community-event-marble-v1.jpg','센터 프로그램','바디챌린지 34기','회원님의 도전 이야기를 만나보세요.','board.html?board=free&category=challenge&view=list'],['community-trainer-v1.jpg','트레이너 소개','이상원 팀장님','나에게 맞는 운동을 시작해 보세요.','index.html#trainer'],['상담실사진.jpg','상담 안내','함께 시작하는 건강한 일상','센터에서 나에게 맞는 운동을 알아보세요.',booking]];
 eventBox.innerHTML=eventItems.map((e,i)=>'<a class="cinema-event-card" '+(i?'hidden':'')+' href="'+e[4]+'"><span class="cinema-event-badge">'+e[1]+'</span><h3>'+e[2]+'</h3><p>'+e[3]+'</p><img src="'+e[0]+'" alt="" loading="lazy"><span class="cinema-event-cta">자세히 보기 ↗</span></a>').join('')+'<div class="cinema-event-controls"><button type="button" aria-label="이전 센터 소식">'+arrow('left')+'</button><span aria-live="polite">1 / 3</span><button type="button" aria-label="다음 센터 소식">'+arrow('right')+'</button></div>';
 let eventIndex=0;const eventShow=delta=>{eventIndex=(eventIndex+delta+eventItems.length)%eventItems.length;eventBox.querySelectorAll('.cinema-event-card').forEach((el,i)=>el.hidden=i!==eventIndex);eventBox.querySelector('.cinema-event-controls span').textContent=(eventIndex+1)+' / 3';};
 eventBox.querySelector('[aria-label="이전 센터 소식"]').addEventListener('click',()=>eventShow(-1));eventBox.querySelector('[aria-label="다음 센터 소식"]').addEventListener('click',()=>eventShow(1));
