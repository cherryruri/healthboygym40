document.addEventListener("DOMContentLoaded", function(){
  ...
  function initHeroVideoLoaderHold(){

    const video =
      document.querySelector(".intro-video");

    if(!video || video.dataset.loaderHoldReady === "true") return;

    video.dataset.loaderHoldReady =
      "true";
    ...
    const holdAtStart =
      ()=>{
        ...
        if(Number.isFinite(duration) && duration > 0){
          video.currentTime =
            Math.min(4, Math.max(0, duration - .25));
        }
  ...
  function initMobileIntroVideo(){
    ...
    const keepMobileLoopAwayFromBlackFrame =
      ()=>{
      ...
      if(duration - video.currentTime > 0.45) return;
    ...
    video.addEventListener(
      "timeupdate",
      keepMobileLoopAwayFromBlackFrame,
      {passive:true}
    );
